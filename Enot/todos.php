...
$due_date, $attachment]);
 ...
 $id = $_POST['id'];
 $old = $pdo->prepare("SELECT status, notifications FROM todos WHERE id = ?")-
>execute([$id]); $old = $pdo->fetch();
...
$count = $pdo->prepare("SELECT COUNT(*) FROM todos t $where")->execute($params); $total = $pdo-
>fetchColumn();
...
<div class="modal fade" id="editModal"><div class="modal-dialog modal-lg"><div class="modal-content">
 ...
 <?php include 'todo_form.php'; ?>
 <div class="modal-footer"><button type="submit" class="btn btn-primary">Update</button><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button></div>
 </form>
</div></div></div>
<script>
function editTodo(t) {
