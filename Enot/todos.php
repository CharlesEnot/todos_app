...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($_POST['action'] === 'add') {
        ...
        // Fetch existing todo row
        $oldStmt = $pdo->prepare("SELECT status, notifications, attachment FROM todos WHERE id = ? AND user_id = ?");
        $oldStmt->execute([$id, $user_id]);
        $old = $oldStmt->fetch(PDO::FETCH_ASSOC);

        if ($status == 'completed' && (!isset($old['status']) || $old['status'] != 'completed') && $notifications) {
        ...
        $currentAttachment = $attachment ?: ($old['attachment'] ?? '');
        $stmt->execute([$category_id, $title, $desc, $status, $priority, $notifications, $due_date, $currentAttachment, $id, $user_id]);
    }
...
// Correctly prepare/execute and fetch the count
$countStmt = $pdo->prepare("SELECT COUNT(*) FROM todos t $where");
...
$params_for_list = array_merge($params, [$limit, $offset]); // avoid mutating original $params if reused
$stmt->execute($params_for_list);
$todos = $stmt->fetchAll(PDO::FETCH_ASSOC);

// Fetch categories for filters/forms
$catsStmt = $pdo->prepare("SELECT * FROM categories WHERE user_id = ?");
        ...
        <div class="col-md-3">
            <select name="category" class="form-select">
                ...
                <?php foreach ($cats as $c): ?>
                    <option value="<?php echo $c['id']; ?>" <?php echo ($_GET['category'] ?? '') == $c['id'] ? 'selected' : ''; ?>><?php echo htmlspecialchars($c['name']); ?></option>
                <?php endforeach; ?>
            </select>
        </div>
        ...
        <tbody>
            <?php foreach ($todos as $t): ?>
                <tr>
                    ...
                    <td><?php echo $t['attachment'] ? '<a href="' . htmlspecialchars($t['attachment']) . '" target="_blank">View</a>' : '—'; ?></td>
                    <td>
...
```// filepath: c:\xampp\htdocs\Enot\todos.php
<?php
session_start();
if (!isset($_SESSION['user_id'])) {
    header("Location: index.php");
